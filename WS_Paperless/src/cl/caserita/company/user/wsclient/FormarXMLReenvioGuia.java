...
public class FormarXMLReenvioGuia {

	private static Documento doc = null;
	 private static Properties prop=null;
		private static String pathProperties;
		
	public String xml(ExmtraDTO emtraDTO,List exdtra,  int docelec, int docref, TptbdgDTO tptbdg, List acteco,TptempDTO tpt){
		String stringxml="";
	...
	public IdDoc idoc(ExmtraDTO emtraDTO, int codele){
	    ...
	    id.setFolio(BigInteger.valueOf(emtraDTO.getNumGuiaDespacho()));
	    //getXMLCalendar
	    //20121203
	...
	public Receptor receptor(TptbdgDTO tptbdg,TptempDTO tptemp){
		...
		receptor.setRznSocRecep(tptemp.getRazonSocial().trim());
		receptor.setGiroRecep("DISTRIBUIDORA");
		receptor.setDirRecep(tptbdg.getDireccion().trim());
		receptor.setCmnaRecep(tptbdg.getDescComuna().trim());
		receptor.setCiudadRecep(tptbdg.getDescCiudad().trim());
		return receptor;
	}
