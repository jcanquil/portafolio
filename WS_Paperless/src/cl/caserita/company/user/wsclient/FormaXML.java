...
import cl.caserita.dte.MedioPagoType;
...
public class FormaXML {
	private static Documento doc = null;
	 private static Properties prop=null;
		private static String pathProperties;
		private static HashMap impuestosSII =null;
	public String xml(VecmarDTO vecmar, List listaTotal, List cldmco, ClmcliDTO clmcli, int docelec, ClcmcoDTO clcmcoDTO, List acteco, TptempDTO tpt, PrdatcaDTO prdat, HashMap impuestoSII){
		String stringxml="";
	...
	public IdDoc idoc(VecmarDTO vecmar, int codele){
		...
		if (codele==33 && !vecmar.getFormaPago().equals("") && vecmar.getFormaPago()!=null && !vecmar.getFormaPago().equals("0") && !vecmar.getFormaPago().equals(" ") ){
			...
			BigInteger pago = new BigInteger("1");
			id.setFmaPago(pago);
		}
			if (codele==39){
	...
	public Totales totales(List clcdia, VecmarDTO vecmar, int docelec, CalculoExentoNetoDTO montos, ClcmcoDTO clcmcoDTO){
		...
		if (docelec==33 ){
			...
			totales.setMntExe(BigInteger.valueOf(clcmcoDTO.getTotalDocumento()));
		}
		ImptoReten imp = null;
		...
		if (docelec==33 ){
			...
			while (iter.hasNext()){
				...
				if (clc.getMontoImpuesto()!=0 && clc.getCodigoImpuesto()!=2){
					...
					//Se comenta por nueva forma de obtencion de impuesto
				...
				if (clc.getCodigoImpuesto()==2){
					
					totales.setIVA(BigInteger.valueOf(clc.getMontoImpuesto()));
				}
...
//			totales.setMntNeto(BigInteger.valueOf(neto2));
	...
	public Detalle detalleDoc2(List cldmco, List clcdia){
		...
		while (iter.hasNext()){
			...
			if (cld.getCodDocumento()!=39 || cld.getCodDocumento()!=43 ){
			...
			if (clddia!=null){
				...
				while (cl.hasNext()){
					...
					if (extari.getCodImpto()!=1){
						//Se comenta por nueva obtencion del impuesto del SII
	...
	public CalculoExentoNetoDTO detalleDoc(List cldmco, List clcdia){
		...
		while (iter.hasNext()){
			...
			detalle.setNmbItem(procesaCaracteres(cld.getDescArticulo().trim()));
			detalle.setQtyItem(BigDecimal.valueOf(cld.getCantidadArticulo()));
			if (cld.getCodigoArticulo()==22 && cld.getPrecioNeto()==0){
				cld.setPrecioNeto(1);
			}else if (cld.getCodigoArticulo()==4632 && cld.getPrecioNeto()==0){
				cld.setPrecioNeto(1);
			}
			...
			if (clddia!=null){
				...
				while (cl.hasNext()){
					...
					if (extari.getCodImpto()!=1){
						//Se comenta por nueva obtenecion de impuestos
						//detalle.getCodImpAdic().add(recuperaTasaSII(String.valueOf(extari.getCodImpto())));
						detalle.getCodImpAdic().add(String.valueOf(obtieneImpuestoSII((extari.getCodImpto()))));
						montoNeto=montoNeto+cld.getMontoCompra();
					}
...
//				}
				...
				if (cld.getCodDocumento()==39 || cld.getCodDocumento()==43){
					...
					System.out.println("Precio Neto Facturas Varias:"+precioNeto);
				}else
				{
					detalle.setPrcItem(BigDecimal.valueOf(cld.getPrecioNeto()));//PRecio
				}
  ...
  public String formaXMLString(Encabezado enca, Documento doc, TptempDTO tpt, PrdatcaDTO prdat, VecmarDTO vecmar){
	 ...
	 if (vecmar.getFormaPago()!=null && vecmar.getFormaPago().length()>0){
	  ...
	  //xmlCas.append("<CmnaOrigen>"+tpt.get.trim()+"</CmnaOrigen>");
	  xmlCas.append("</Emisor>");
	  ...
	  if (enca.getReceptor().getCmnaRecep().length()>0){
		  xmlCas.append("<CmnaRecep>"+enca.getReceptor().getCmnaRecep()+"</CmnaRecep>");

	  }else{
		  xmlCas.append("<CmnaRecep>"+"SC"+"</CmnaRecep>");
	  }
	  if (enca.getReceptor().getCiudadRecep().length()>0){
		  xmlCas.append("<CiudadRecep>"+enca.getReceptor().getCiudadRecep()+"</CiudadRecep>");

	  }else{
		  xmlCas.append("<CiudadRecep>"+"SC"+"</CiudadRecep>");
...
//	  }
	  ...
	  if (prdat!=null){
System.out.println("Ingresa por numero OC y FECHA OC");
if (prdat.getNumeroOrden()!=null){
	System.out.println("Ingresa por numero OC y FECHA OC2");

			  if (prdat.getNumeroOrden().trim().length()>0){
				  System.out.println("Ingresa por numero OC y FECHA OC3");

				  String numero=prdat.getNumeroOrden().trim();
				  String fechaOC=prdat.getFechaOrden().trim();
				  xmlCas.append("<Referencia>");
					  xmlCas.append("<NroLinRef>"+1+"</NroLinRef>");
					  xmlCas.append("<TpoDocRef>"+801+"</TpoDocRef>");
					  xmlCas.append("<FolioRef>"+numero+"</FolioRef>");
					  xmlCas.append("<FchRef>"+fechaOC+"</FchRef>");
				  xmlCas.append("</Referencia>"); 
			  }
		  }
  ...
  public static void main (String args[]){
	  //<Documento ID="">
	  //<Documento ID="">
	  String xmp="<Documento ID="+"\""+"\">";
	  
	  System.out.println("XML:"+xmp);
