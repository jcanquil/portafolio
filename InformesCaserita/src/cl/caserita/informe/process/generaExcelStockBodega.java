...
public class generaExcelStockBodega {
	...
	public String generaExcelPrecio(List lista){
		...
		while (iter.hasNext()){
			...
			ExmarbDTO ges = new ExmarbDTO();
			ges = (ExmarbDTO) iter.next();
		...
		try{
			...
			FileOutputStream archivo = new FileOutputStream("/home/ServiciosCaserita/informesPrecio/INFORME_PRECIO_"+ fechaStr +".xls");
			
			nombreArchivo="INFORME_PRECIO_" + fechaStr+".xls";
			String directorio = "/home/ServiciosCaserita/informesPrecio/";
	...
	public List llenaListaUnica(ExmarbDTO dato){
		...
		for (int i=0;i<=cant2;i++){
			...
			if (cant==0){
				...
				unico.setDescripcion(String.valueOf(dato.getCodigoArticulo()));
			}else if (cant==2){
				unico.setDescripcion(String.valueOf(dato.getStockLinea()));
			}else if (cant==3){
